...
`SK_EXTRACT_KEYWORDS` / `SK_PROJECT_EXTRACT_KEYWORDS`（issue #79 で追加）はキーワード行抽出機能専用のストレージで、キーワードハイライトの `SK_KEYWORDS` / `SK_PROJECT_KEYWORDS` とは独立しています。`SK_EXTRACT_KEYWORDS` は全体設定のキーワードの JSON エンコードされた配列 `{ id, keyword, fileFilter, caseSensitive, addedOnly, enabled }[]`（`loadGlobalExtractKeywords()` / `saveGlobalExtractKeywords()`）、`SK_PROJECT_EXTRACT_KEYWORDS` はプロジェクト毎のキーワードを保持する JSON エンコードされたマップ `{ [projectId]: entry[] }`（`loadProjectExtractKeywords(projectId)` / `saveProjectExtractKeywords(projectId, keywords)`）です。スコープの考え方・`loadExtractKeywords()`（マージ済みビューを返す）・`moveExtractKeywordScope()`・プロジェクト削除時の `deleteProjectExtractKeywords()` は、いずれも上記のキーワードハイライトの仕組みと同じパターンに倣っています。色やカウントON/OFFといったハイライト用のフィールドは持ちません。`fileFilter`（issue #92）はファイルパスに対する部分一致（常に大文字小文字を区別しない）の絞り込み文字列で、空文字列（未指定・#92 以前に保存されたエントリの既定値、`sanitizeExtractKeywords()` が補完）なら全ファイルが対象になります。`caseSensitive`（issue #95、真偽値）はキーワード一致（`fileFilter` の一致には影響しません）で大文字小文字を区別するかどうかで、`sanitizeExtractKeywords()` は未設定値を `false` として扱います（既存エントリ・新規エントリともデフォルトは区別しない＝従来通りの挙動）。`extractKeywordMatches(raw, fileFilter, caseSensitive, addedOnly)` がこのフラグに応じて比較前の `toLowerCase()` をスキップします。UIではキーワード入力欄の隣の「Aa」チェックボックスで切り替えます。`addedOnly`（issue #106、真偽値）は抽出対象を追加行（`+` で始まる行）のみに絞り込むかどうかで、`sanitizeExtractKeywords()` は未設定値を `false` として扱います（既存エントリ・新規エントリともデフォルトは追加行・削除行の両方が対象＝従来通りの挙動）。`extractKeywordMatches()` はこのフラグが true のとき `-` で始まる行をスキップします。UIではキーワード入力欄の隣の「+のみ」チェックボックス（`buildExtractKeywordRow()`）で切り替え、有効な場合は抽出結果の見出しに `[追加行のみ]` が付加されます（`buildExtractResultSection()`）。また各抽出結果の見出しには「📋 コピー」ボタンがあり、`copyExtractMatchesToClipboard()` がその欄のマッチ行を `filePath: +/-content` 形式（1行1マッチ）でクリップボードにコピーします（`navigator.clipboard.writeText()`、外部通信は発生しません）。`enabled`（issue #109、真偽値）はそのキーワードの抽出を有効にするかどうかで、`sanitizeExtractKeywords()` は未設定値を `true` として扱います（既存エントリ・新規エントリともデフォルトは有効）。`renderExtractResults()` は `enabled: false` のエントリを抽出結果一覧から丸ごと除外します（行自体を作りません）。UIでは各行先頭のチェックボックス（`buildExtractKeywordRow()`）で個別に切り替えられるほか、モーダル上部の「全て有効」「全て無効」ボタン（`setAllExtractKeywordsEnabled(enabled)`）で、現在表示中の全キーワード（全体設定 + アクティブプロジェクト自身のキーワード）の `enabled` を一括変更できます。
...
```json
{
  ...
  "extractKeywords": [
    { "id": "exkw_...", "keyword": "TODO" }
  ],
  "projectExtractKeywords": {
    "projectId": [
...
│                        （プロジェクトID → 配列）      │
│  gitLocalReview_extractKeywords 行抽出キーワード配列  │
│                                （全体設定）             │
...
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       ...
       └─────────────────┴─────────────────┘
         ...
         として保存           │           │
              │          単一ファイル   複数ファイル一括
              │        showConflictModal()  最終更新日時が最も新しい
...
複数ファイル一括読み込み時に同名の既存プロジェクトと衝突した場合は、`showConflictModal()` を出さずに `loadDiffFile()` の `batchMode` が自動的に処理する（`updateExistingProject()` を呼び出し、最終更新日時が最も新しい同名プロジェクトを更新する）。単一ファイル読み込み時のみ、モーダルでユーザーに更新先プロジェクトの選択（または新規作成）を委ねる。

同じバッチ内に同名ファイルが複数含まれる場合（バッチ開始前には存在しなかった同名プロジェクト同士）は、後続のファイルが先に作成されたプロジェクトを上書きしないよう、`loadDiffFiles()` がバッチ全体で共有する `batchCreatedIds`（このバッチ内で新規作成したプロジェクトIDの集合）を `loadDiffFile()` に渡し、自動更新の対象から除外する。この場合、後続の同名ファイルはそれぞれ独立した新規プロジェクトとして作成される。
