...
`SK_EXTRACT_KEYWORDS` / `SK_PROJECT_EXTRACT_KEYWORDS`（issue #79 で追加）はキーワード行抽出機能専用のストレージで、キーワードハイライトの `SK_KEYWORDS` / `SK_PROJECT_KEYWORDS` とは独立しています。`SK_EXTRACT_KEYWORDS` は全体設定のキーワードの JSON エンコードされた配列 `{ id, keyword, fileFilter }[]`（`loadGlobalExtractKeywords()` / `saveGlobalExtractKeywords()`）、`SK_PROJECT_EXTRACT_KEYWORDS` はプロジェクト毎のキーワードを保持する JSON エンコードされたマップ `{ [projectId]: entry[] }`（`loadProjectExtractKeywords(projectId)` / `saveProjectExtractKeywords(projectId, keywords)`）です。スコープの考え方・`loadExtractKeywords()`（マージ済みビューを返す）・`moveExtractKeywordScope()`・プロジェクト削除時の `deleteProjectExtractKeywords()` は、いずれも上記のキーワードハイライトの仕組みと同じパターンに倣っています。色やカウントON/OFFといったハイライト用のフィールドは持たず、エントリは `{ id, keyword, fileFilter }` のみです。`fileFilter`（issue #92）はファイルパスに対する部分一致（大文字小文字を区別しない）の絞り込み文字列で、空文字列（未指定・#92 以前に保存されたエントリの既定値、`sanitizeExtractKeywords()` が補完）なら全ファイルが対象になります。
